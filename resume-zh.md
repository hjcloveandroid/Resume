...
从事KaiOS(基于Firefox)应用的客户支持工作:
- 电话业务相关包含联系人，通话，邮件等... 
- 海外运营商特殊需求，主要包含sim卡客制化相关和电信业务(场测, 协议一致性, 运营商入网入库测试相关)等...
- gaia上层应用重构(preact)，优化首次进入的应用的时间，添加单元测试(jest+enzyme)
...
责任描述: 使用React技术栈重写message应用, 使用web idl实现对应的接口(行程码部分)
```
...
责任描述: 负责整体项目的评估, 开发部分负责callscreen界面编码工作以及和服务器数据交互部分
```
